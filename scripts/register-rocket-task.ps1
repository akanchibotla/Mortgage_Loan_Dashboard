...
.BEHAVIOUR
  - Weekly trigger (Sunday 18:00 local) with -StartWhenAvailable: if the
    device was off/asleep at the trigger time, the task runs as soon as the
    device is next available. That is what makes "have a device on at least
    once a month" sufficient.
  - Runs only when a network is available; 15-minute hard time limit.
  - User-level task (no admin/elevation, no stored password): runs when you
    are logged on, under your own git credentials.
