...
try {
    ...
    for ($i = 1; $i -le 5; $i++) {
        ...
        if ($LASTEXITCODE -ne 0) {
            # A rebase conflict here would leave the tree mid-rebase, and a
            # blind retry could push conflict markers into the public data.
            # Abort to a clean state and bail; the next scheduled run retries
            # from scratch. This only arises if another device diverged the
            # SAME date's row via a Wayback fallback -- live Tier-1 rows are
            # identical, so the common concurrent case stays conflict-free.
            Write-Log 'Rebase failed (likely a concurrent divergent write); aborting to a clean state.' 'ERROR'
